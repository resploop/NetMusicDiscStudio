...
- 取出时**整张专辑一起取出**，不会只抽走一首、剩下几首散落在队列里

实现上，网络CD是在**放进 MP4 的那一刻**被摊平成 N 条队列项的（每条对应专辑里的一首），
所以切歌与循环用的完全是上游原有那套逻辑，本模组只负责把它们在界面上重新折叠回一行。

> **上限**：MP4 队列本身最多 18 条（上游 `MP4Item.MAX_QUEUE_SIZE`），所以一张专辑在 MP4 里
> 最多放 18 首，超出部分会在放入时截断并写一条日志。上游的唱片机没有这个限制。
